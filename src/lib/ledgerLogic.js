...
export function generateLedger(currentData, spendingStrategy = 'fixed', guardrails = {}) {
  ...
  const balances = {
    ...
    hsaSpouse: hsaSpouse,
    realEstate:
      realEstate && Array.isArray(realEstate) && realEstate.length > 0
  ...
  if (credit?.reverseMortgage?.enabled) {
    ...
    balances.hecmLimit = calculateInitialPrincipalLimit(
      startAge,
      hecmParams.homeValue,
      hecmParams.growthRate
    );
  }

  let brokerageBasis =
    assets.brokerageBasis?.joint ||
    assets.brokerageBasis?.client ||
    assets.brokerageBasis?.spouse ||
    0;
  ...
  const annualTLH =
    (taxOptimization?.taxLossHarvesting?.crypto || 0) +
    (taxOptimization?.taxLossHarvesting?.brokerage || 0);
  ...
  for (let i = 0; i < loopDuration; i++) {
    // loopDuration is already calculated to be (endAgeSafe - startAgeSafe + 1) + shockLongevity
    ...
    if (isSpouseDeathYear || isClientDeathYear) {
      ...
      console.log(
        `Survivor Logic: Basis Step-Up Triggered in ${currentYear}. Community=${isCommunity}, NewBasis=${brokerageBasis}`
      );
    }
    ...
    const getMonthsEligible = (bDate, startAge, year) => {
      ...
      // Add remaining months (fraction of age * 12)
      const additionalMonths = Math.round((startAge % 1) * 12);
      claimDate.setMonth(claimDate.getMonth() + additionalMonths);

      // Current Year Window
      const yearStart = new Date(year, 0, 1);
      const yearEnd = new Date(year, 11, 31); // Dec 31

      // If we claim after this year ends, 0 months
      if (claimDate > yearEnd) return 0;

      // If we claimed before this year starts, 12 months
      if (claimDate < yearStart) return 12;

      // Otherwise, we claim DURING this year.
      // Months eligible = (12 - ClaimMonthIndex).
      // Ex: Claim in Jan (0). 12-0 = 12 months.
      // Ex: Claim in Dec (11). 12-11 = 1 month.
      // However, SS is paid next month? Let's assume eligibility starts month of claim.
      return Math.max(0, 12 - claimDate.getMonth());
    };
    ...
    const currentPortfolio =
      balances.traditionalClient +
      balances.traditionalSpouse +
      balances.rothClient +
      balances.rothSpouse +
      balances.brokerage +
      balances.hsaClient +
    ...
    let discretionaryBase =
      (expenses.discretionaryMonthly || 0) * 12 + (expenses.discretionary || 0);
    ...
    const discretionaryInflated =
      discretionaryBase * phaseFactor * Math.pow(1 + computedInflation / 100, i);
    ...
    if (isRetired) {
      switch (spendingStrategy) {
        case 'blanchett':
          effectiveAnnualExpenses = Math.max(
            minimumNeed,
            calculateBlanchettSmile(clientAge, client.retirementAge, annualExpenses)
          );
          break;
        case 'guardrails':
          effectiveAnnualExpenses = Math.max(
            minimumNeed,
            applyGuardrails(currentPortfolio, peakPortfolioValue, annualExpenses, guardrails)
          );
          break;
        case 'percentage':
          effectiveAnnualExpenses = Math.max(
            minimumNeed,
            calculatePercentageBased(currentPortfolio, spendingParams.percentageRate || 0.04)
          );
          break;
        case 'floor-ceiling': {
          const floor =
            (spendingParams.floorAmount || 50000) * Math.pow(1 + computedInflation / 100, i);
          const ceiling =
            (spendingParams.ceilingAmount || 200000) * Math.pow(1 + computedInflation / 100, i);
          const effectiveFloor = Math.max(floor, minimumNeed);
          effectiveAnnualExpenses = calculateFloorCeiling(
            currentPortfolio,
            spendingParams.percentageRate || 0.04,
            effectiveFloor,
            ceiling
          );
          break;
        ...
        case 'dynamic':
          effectiveAnnualExpenses = Math.max(
            minimumNeed,
            calculateActuarial(currentPortfolio, clientAge, client.lifeExpectancy || 95)
          );
          break;
        case 'max-spend': {
          ...
          if (clientAge >= maxStart && clientAge < maxEnd) {
            const maxSpendCalc = calculateMaxSpend(
              currentPortfolio,
              clientAge,
              maxEnd,
              0.05,
              spendingParams.maxSpendLegacy || 0
            );
        ...
        default:
          ...
          if (expenses.flexibility?.enabled && peakPortfolioValue > 0) {
            ...
            if (drop > (expenses.flexibility.portfolioDropThreshold || 0.1)) {
              const rawEss = expenses.essentialMonthly
                ? expenses.essentialMonthly * 12
                : expenses.essential || 0;
              const rawDisc = expenses.discretionaryMonthly
                ? expenses.discretionaryMonthly * 12
                : expenses.discretionary || 0;
              const totalRaw = rawEss + rawDisc;
              const discRatio = totalRaw > 0 ? rawDisc / totalRaw : 0;
              const cutPercent = expenses.flexibility.discretionaryCut || 0.3;
    ...
    currentRealEstate.forEach((re) => {
      ...
      if (re.mortgageBalance > 0 && re.mortgage) {
        ...
        if (re.mortgage.targetAge && re.mortgage.targetAge > clientAge) {
          ...
          extraPayment = Math.max(
            0,
            (re.mortgageBalance - annualPayment * yearsRemaining) / yearsRemaining
          );
        }
    ...
    if ((clientAlive && clientAge < 65) || (spouseAlive && spouseAge < 65)) {
      ...
      const acaParams = assumptions.aca || {
        benchmarkPremium: 600,
        householdSize: spouseAlive ? 2 : 1,
        povertyLine: 14580,
      };
      const result = calculateACASubsidy(
        acaMagi,
        spouseAlive ? 2 : 1,
        acaParams.benchmarkPremium,
        acaParams.povertyLine
      );
    ...
    if (currentData.stressTest?.ltcEvent?.enabled) {
      ...
      if (clientAge >= ltcStart && clientAge < ltcStart + ltcYears) {
        ltcCost =
          (currentData.stressTest.ltcEvent.monthlyCost || 0) *
          12 *
          Math.pow(1 + computedInflation / 100, i);
    ...
    const cryptoLocInterest = balances.cryptoLOC * (credit?.cryptoLOC?.rate || 0.1);
    ...
    let annualLocBorrow = 0,
      annualCryptoLocBorrow = 0,
      annualHecmBorrow = 0;
    ...
    if (bucketStrategy?.enabled) {
      const annualReturn =
        (isRetired ? assumptions.retirementReturn : assumptions.preRetirementReturn) || 6;
      const targetCash = effectiveAnnualExpenses * (bucketStrategy.yearsOfCash || 2);
      if (
        shouldRefillBucket({
          currentCash: balances.cash,
          targetCash,
          portfolioReturn: annualReturn / 100,
          refillConditions: bucketStrategy.refillCondition,
        })
      ) {
    ...
    if (rawDeficit > 0) {
      ...
      const sources = [
        {
          id: 'brokerageLOC',
          coll: balances.brokerage,
          settings: credit?.brokerageLOC,
          balance: balances.brokerageLOC,
        },
        {
          id: 'cryptoLOC',
          coll: balances.crypto,
          settings: credit?.cryptoLOC,
          balance: balances.cryptoLOC,
        },
      ];
      if (
        credit?.reverseMortgage?.enabled &&
        clientAge >= (credit.reverseMortgage.ageStart || 62)
      ) {
        sources.push({
          id: 'hecm',
          coll: balances.hecmLimit,
          isHECM: true,
          balance: balances.hecmBalance,
          settings: { rate: 0 },
        });
      }
      ...
      for (const src of sources) {
        ...
        if (src.settings || src.isHECM) {
          let maxDraw = src.isHECM
            ? src.coll
            : getLOCOffer({
                collateralValue: src.coll,
                ltv: src.settings.ltv,
                rate: src.settings.rate,
              }).maxDraw;
          const available = Math.max(0, maxDraw - src.balance);
          let should = src.isHECM
            ? true
            : shouldBorrow({
                expectedReturn: 0.08,
                rateAfterTax: src.settings.rate,
                marginProb: 0.01,
              });
    ...
    const totalPropTax =
      currentRealEstate.reduce((sum, re) => sum + (re.propertyTax || 0), 0) * housingInf;
    const itemizedItems = {
      medical: healthcareCost + (expenses.medical || 0),
      mortgageInterest: totalMortInterest,
      charity: itemizedCharity,
    ...
    if (taxFreeEnabled && isRetired) {
      const withdrawals = optimizeTaxFreeWithdrawal(
        gapPostBucket,
        {
          traditional: {
            client: balances.traditionalClient - totalRMD,
            spouse: balances.traditionalSpouse,
          },
      ...
      const taxes = calculateTotalTax({
        ...
        stateRate: currentData.stateTaxRate || 0,
        isRetired,
        itemizedItems,
        year: currentYear,
        enableTCJASunset,
      ...
      result = optimizeWithdrawals({
        ...
        filingStatus: currentFilingStatus,
        ordinaryIncome: salary + totalRMD,
        ssBenefits: ss,
        qualifiedDividends: qualDividends,
        ordinaryDividends: ordDividends,
        brokerageBasis,
        taxLossHarvesting: lossBank,
        strategy: {
          ...taxOptimization,
          allowRothConversion:
            taxOptimization.enableRothConversion ||
            taxOptimization.rothStrategy === '12' ||
            taxOptimization.rothStrategy === '22' ||
            taxOptimization.rothStrategy === '24',
        ...
        year: currentYear,
        enableTCJASunset,
        enableDynamicMAGI: taxOptimization?.enableDynamicMAGI,
      });
    }
    ...
    const totalTradOutflow =
      taxFreeEnabled && isRetired
    ...
    if (withdrawals.brokerage > 0) {
      ...
      if (oldTotal > 0) {
        brokerageBasis = Math.max(
          0,
          brokerageBasis - withdrawals.brokerage * (brokerageBasis / oldTotal)
        );
      }
    }
    ...
    const maxCrypto = assumptions.maxCryptoAllocation || 0.2; // Default 20% cap to prevent runaway
    ...
    if (balances.crypto > 0 && cryptoAlloc > maxCrypto) {
      ...
      if (sellAmt > 1000) {
        ...
        // Basis Ratio
        const basisRatio =
          balances.crypto + sellAmt > 0 ? cryptoBasis / (balances.crypto + sellAmt) : 0;
    ...
    const totalDebt =
      currentRealEstate.reduce((sum, re) => sum + re.mortgageBalance, 0) + balances.hecmBalance;
    ...
    const effectiveLegacy =
      grossEstate - totalDebt - estateTax + (isILIT ? deathBenefit : 0) - irdTax;
    ...
    const discretionaryExpenses =
      safeNum(expenses.discretionary) *
      (i === 0 ? 1 : Math.pow(1 + safeNum(assumptions?.inflationRate) / 100, i));
    ...
    ledger.push({
      ...
      metrics: {
        ...
        yearlyNetDifference:
          i > 0 ? portVal + totalRealEstateValue - totalDebt - ledger[i - 1].netWorth : 0,
    ...
    if (i > 0) {
      ledger[i].metrics.cumulativeAssetGrowth =
        ledger[i - 1].metrics.cumulativeAssetGrowth + ledger[i].metrics.yearlyAssetGrowth;
    }
...
export { calculateCryptoBalance, calculateExpenses };
