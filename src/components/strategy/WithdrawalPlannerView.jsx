...
export default function WithdrawalPlannerView({ darkMode = false }) {
    ...
    const options = {
        ...
        scales: {
            ...
            y: {
                stacked: true,
                title: { display: true, text: 'Withdrawal Amount ($)', color: darkMode ? '#f3f4f6' : '#1f2937' },
                ticks: {
    ...
    if (!ledger || ledger.length === 0) {
        return (
            <div className="bg-white dark:bg-gray-800 p-8 rounded-xl border border-dashed border-gray-300 dark:border-gray-700 text-center">
                <p className="text-gray-500 dark:text-gray-400 italic">Run calculation to visualize your multi-year withdrawal plan...</p>
            </div>
        );
    }
                                    ...
                                    return (
                                        <tr key={idx} className="hover:bg-blue-50/50 dark:hover:bg-blue-900/10 transition-colors">
                                            ...
                                            <td className="px-3 py-2 text-right">
                                                <span className={`px-1.5 py-0.5 rounded font-bold ${taxRate <= 5 ? 'bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-300' : 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/40 dark:text-yellow-300'}`}>
                                                    {taxRate.toFixed(1)}%
                                                </span>
                                            </td>
                                        </tr>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <ol className="text-[10px] text-gray-600 dark:text-gray-400 space-y-1 list-decimal ml-4">
                            <li><strong>Standard Deduction Check:</strong> Fills Bucket #1 (Traditional) up to SD amount ($0 Tax).</li>
                            <li><strong>0% Capital Gains:</strong> Sells Brokerage/Crypto to realize gains up to 0% bracket limit.</li>
                            <li><strong>Gap Filling:</strong> Uses Roth assets to cover any remaining expenses (Invisible to IRS).</li>
                            <li><strong>Last Resort:</strong> If Roth is exhausted, pulls from Traditional (Subject to Ordinary Tax).</li>
                        </ol>
                    </div>
                    <div className="flex flex-col justify-center">
                        <div className="p-4 bg-white dark:bg-gray-800 rounded-lg border border-blue-200 dark:border-blue-800 shadow-sm">
                            <div className="flex items-center justify-between mb-2">
                                <span className="text-xs font-bold text-gray-700 dark:text-gray-300">Lifetime Withdrawal Efficiency</span>
