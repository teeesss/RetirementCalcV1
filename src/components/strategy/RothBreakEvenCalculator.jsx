...
export default function RothBreakEvenCalculator({ darkMode }) {
  ...
  const [capGainsRate] = useState(15);
  ...
  return (
    <div
      className={`p-4 rounded-lg border flex flex-col gap-4 ${darkMode ? 'bg-gray-700/50 border-gray-600' : 'bg-purple-50 border-purple-100'}`}
    >
      ...
      <div
        className={`p-3 rounded border text-center ${isFavorable ? 'bg-green-100 border-green-200 text-green-800' : 'bg-yellow-100 border-yellow-200 text-yellow-800'}`}
      >
