      ...
      {!isRetired ? (
        <div className="bg-blue-50 dark:bg-blue-900/20 p-6 rounded-lg border border-blue-200 dark:border-blue-800 text-center">
          ...
          <p className="text-sm text-gray-700 dark:text-gray-300">
            MAGI monitoring becomes critical after retirement to maximize ACA subsidies and minimize
            IRMAA surcharges.
          </p>
        </div>
      ) : (
        <>
            ...
            >
              ...
              <div className="text-xs text-gray-600 dark:text-gray-400 mt-2">
                {qualifiesForACA
                  ...
                  : magiAsPercentFPL < acaSubsidyMin
                    ? 'MAGI too low'
                    : 'MAGI too high'}
              </div>
            </div>
                ...
                <div className="flex-1">
                  ...
                  <div className="bg-white dark:bg-gray-800 p-3 rounded border border-green-200 dark:border-green-800 text-sm">
                    ...
                    <p className="text-gray-700 dark:text-gray-300">
                      Keep MAGI between {acaSubsidyMin}-{acaSubsidyMax}% FPL ($
                      {(fpl * (acaSubsidyMin / 100)).toLocaleString()} - $
                      {(fpl * (acaSubsidyMax / 100)).toLocaleString()}) to maximize ACA subsidies.
                      Use Roth withdrawals (invisible to MAGI) instead of Traditional IRA when at
                      risk of exceeding {acaSubsidyMax}% FPL.
                    </p>
                  </div>
                </div>
              </div>
          ...
          {irmaaAge &&
            nextIRMAABracket &&
            distanceToNextBracket > 0 &&
            distanceToNextBracket < 50000 && (
                  ...
                  <div className="flex-1">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-3">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-gray-600 dark:text-gray-400">
                          MAGI increase triggers +$
                          {(nextIRMAABracket.surcharge - currentIRMAABracket.surcharge) * 12}/yr
                        </div>
                      </div>
                    </div>
                    <div className="bg-white dark:bg-gray-800 p-3 rounded border border-yellow-300 dark:border-yellow-700 text-sm">
                      ...
                      <p className="text-gray-700 dark:text-gray-300">
                        {"You're only"} ${distanceToNextBracket.toLocaleString()} {'away from a'} $
                        {(
                          (nextIRMAABracket.surcharge - currentIRMAABracket.surcharge) *
                          12
                        ).toLocaleString()}
                        {'/year Medicare surcharge increase!'}
                        Use Roth withdrawals instead of Traditional IRA to avoid crossing the IRMAA
                        threshold.
                      </p>
                    </div>
                  </div>
          ...
          {irmaaAge && lookbackYear && (
            <div className="bg-white dark:bg-gray-800 p-6 rounded-lg border border-gray-200 dark:border-gray-700">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded border border-blue-200 dark:border-blue-800">
                  <div className="text-sm text-gray-600 dark:text-gray-400 mb-1">
                    MAGI from {lookbackYear.age - currentAge + currentAge - 2} years ago (Age{' '}
                    {lookbackYear.age})
                  </div>
              ...
              <div className="mt-4 p-3 bg-gray-50 dark:bg-gray-900/50 rounded text-sm">
                ...
                <p className="text-gray-700 dark:text-gray-300">
                  IRMAA surcharges are based on your MAGI from 2 years ago. {"Today's"} Roth
                  conversions and withdrawals will affect your Medicare premiums in {currentAge + 2}
                  , so plan accordingly!
                </p>
              </div>
            </div>
