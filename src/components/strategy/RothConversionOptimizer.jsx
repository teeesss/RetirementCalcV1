...
export default function RothConversionOptimizer() {
  ...
  const ssIncome =
    currentAge >= (planData.socialSecurity?.primary?.startAge || 67)
  ...
  const traditionalBalance =
    (planData.assets?.traditional?.client || 0) + (planData.assets?.traditional?.spouse || 0);
  ...
  const standardDeduction =
    filingStatus === 'married' ? 30000 : filingStatus === 'head' ? 22500 : 15000;
    ...
    <div className="bg-white dark:bg-gray-800 p-6 rounded-lg border border-gray-200 dark:border-gray-700">
      <div className="flex items-center justify-between mb-2">
        ...
        <div className="flex items-center gap-3">
          <span
            className={`px-2 py-1 rounded text-xs font-bold ${isEnabled ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}
          >
      ...
      <p className="text-sm text-gray-600 dark:text-gray-400 mb-6">
        Build your &quot;invisible money&quot; bucket by converting Traditional IRA funds to Roth
        annually
      </p>
          ...
          <div className="space-y-3">
            ...
            {conversion.amount === 0 && (
              <div className="p-2 bg-yellow-100 text-yellow-800 text-xs rounded border border-yellow-200">
                <strong>Why $0?</strong> Your current income (${currentIncome.toLocaleString()})
                combined with the standard deduction (${standardDeduction.toLocaleString()}) likely
                already fills or exceeds the {targetBracket}% bracket. Consider raising the target
                bracket to finding conversion room.
              </div>
            )}
            ...
            <div className="pt-3 border-t border-green-200 dark:border-green-800">
              ...
              <div className="text-xs text-green-700 dark:text-green-300">{conversion.benefit}</div>
            </div>
      ...
      <div className="mt-6 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
        ...
        <div className="text-sm text-gray-700 dark:text-gray-300 space-y-2">
          <p>
            <strong>The Goal:</strong> Move money from your Pre-Tax bucket (Traditional IRA) to your
            Tax-Free bucket (Roth IRA).
          </p>
          <p>
            <strong>Why Now:</strong> If you&apos;re in a low tax bracket now, pay {targetBracket}%
            tax today to avoid potentially 22-37% tax in retirement when Required Minimum
            Distributions (RMDs) kick in.
          </p>
          <p>
            <strong>The Magic:</strong> Once in the Roth, this money grows tax-free forever and
            withdrawals are &quot;invisible&quot; to the IRS - they don&apos;t count as income,
            don&apos;t affect ACA subsidies, and don&apos;t trigger IRMAA Medicare surcharges.
          </p>
          <p className="text-green-700 dark:text-green-300 font-semibold">
            Convert ${conversion.amount.toLocaleString()} annually and you&apos;ll build a
            substantial &quot;invisible money&quot; war chest for tax-free retirement living!
          </p>
        </div>
      </div>
    </div>
