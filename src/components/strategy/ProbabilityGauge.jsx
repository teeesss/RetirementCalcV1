...
export default function ProbabilityGauge({ successRate = 0, iterations = 0 }) {
  ...
  const getFillColor = (rate) => {
    if (rate >= 0.9) return '#10b981'; // green-500
    if (rate >= 0.75) return '#eab308'; // yellow-500
    if (rate >= 0.6) return '#f97316'; // orange-500
    ...
    <div className="flex flex-col items-center justify-center p-6 bg-white dark:bg-gray-800 rounded-lg shadow-md">
      ...
      <div className="relative w-48 h-48">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <div className={`text-5xl font-bold ${getColor(successRate)}`}>{percentage}%</div>
      ...
      <div className="mt-4 text-center">
        <div className="text-xs text-gray-600 dark:text-gray-400">
          {successRate >= 0.9 && '🎯 Excellent Plan'}
          {successRate >= 0.75 && successRate < 0.9 && '✅ Good Plan'}
          {successRate >= 0.6 && successRate < 0.75 && '⚠️ Moderate Risk'}
          {successRate < 0.6 && '🚨 High Risk'}
        </div>
      </div>
    </div>
  );
