...
export default function ScenarioComparisonChart({ darkMode = false }) {
    const { scenarios, ledger, calculateSuccess, monteCarloResults, planData } = usePlan();
    const { comparisonBaseline, setComparisonBaseline, comparisonProposed, setComparisonProposed } = useTaxStrategy();

    // Helper to get KPIs from active plan if it's selected as 'Active Plan'
    const getActivePlanMetrics = () => {
        ...
        return {
            ...
            legacyValue: final?.legacyValue || 0, // v1.5
            annualTaxRates: ledger.map(y => y.taxes?.effectiveRate || 0) // v1.5
        };
    };

    const allOptions = [
        getActivePlanMetrics(),
        ...scenarios.map(s => ({
            id: s.id,
            name: s.name,
            ...
            legacyValue: s.kpis?.annualLegacy ? s.kpis.annualLegacy[s.kpis.annualLegacy.length - 1] : (s.kpis?.finalBalance * 0.95), // Estimate if missing
    ...
    }, [scenarios, comparisonBaseline, comparisonProposed, setComparisonBaseline, setComparisonProposed, allOptions]); // Logic to init
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
                <div>
                    <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Baseline Plan</label>
                    <select
                ...
                <div>
                    <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Proposed Plan</label>
                    <select
                        className="w-full text-sm p-2 rounded border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm"
            ...
            <div className="p-4 bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700">
                ...
                <div className="mt-2 text-[10px] text-gray-500 text-center italic">
                    Visualizes the "Widow Spike" where tax rates may jump after a spouse passes away.
                </div>
            </div>

            {/* Difference Summary */}
            <div className="grid grid-cols-3 gap-3">
                <div className={`p-3 rounded-lg border ${wealthDiff >= 0 ? 'border-green-300 bg-green-50 dark:bg-green-900/20' : 'border-red-300 bg-red-50 dark:bg-red-900/20'}`}>
                    <div className="text-[10px] text-gray-500 uppercase font-bold">Wealth Delta</div>
                    <div className={`text-lg font-mono font-bold ${wealthDiff >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                        {wealthDiff >= 0 ? '+' : ''}{formatCompactCurrency(wealthDiff)}
                    </div>
                </div>

                <div className={`p-3 rounded-lg border ${taxDiff <= 0 ? 'border-blue-300 bg-blue-50 dark:bg-blue-900/20' : 'border-red-300 bg-red-50 dark:bg-red-900/20'}`}>
                    <div className="text-[10px] text-gray-500 uppercase font-bold">Tax Alpha (Savings)</div>
                    <div className={`text-lg font-mono font-bold ${taxDiff <= 0 ? 'text-blue-600 dark:text-blue-400' : 'text-red-600'}`}>
                        {taxDiff <= 0 ? 'Saved ' : 'Extra '}{formatCompactCurrency(Math.abs(taxDiff))}
                    </div>
                </div>

                <div className={`p-3 rounded-lg border ${legacyDiff >= 0 ? 'border-green-300 bg-green-50 dark:bg-green-900/20' : 'border-red-300 bg-red-50 dark:bg-red-900/20'}`}>
                    <div className="text-[10px] text-gray-500 uppercase font-bold">Legacy Value</div>
                    <div className={`text-lg font-mono font-bold ${legacyDiff >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                        {legacyDiff >= 0 ? '+' : ''}{formatCompactCurrency(legacyDiff)}
                    </div>
                </div>
            </div>
        </div>
