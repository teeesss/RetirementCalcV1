...
export default function MarginalTaxChart({ darkMode = false }) {
  ...
  const chartData = useMemo(() => {
    ...
    const ss =
      ...
      (planData.socialSecurity?.spouse?.annualAmount || 0);
  ...
  const options = {
    ...
    scales: {
      ...
      x: {
        title: {
          display: true,
          text: 'Ordinary Income (Withdrawals + Wages)',
          color: darkMode ? '#9ca3af' : '#6b7280',
        },
        ticks: {
          color: darkMode ? '#9ca3af' : '#6b7280',
  ...
  return (
    <div className={`p-4 rounded-lg shadow-md ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
      ...
      <p className="mt-2 text-xs text-center text-gray-500">
        Shows the effective tax rate on the <b>next $100 withdrawn</b>. Note the &quot;Tax
        Torpedo&quot; effect where Social Security becomes taxable, followed by IRMAA cliffs.
      </p>
    </div>
  );
}
