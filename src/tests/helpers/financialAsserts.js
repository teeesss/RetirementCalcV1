...
import { expectMoneyClose, TOLERANCE_PRESETS } from './tolerance.js';
import {
  reportAnomaly,
  SEVERITY,
  detectInvalidNumbers,
  detectNegativeValues,
} from './anomalyReporter.js';
...
export function assertBalanceContinuity(
  year,
  prevYear,
  options = TOLERANCE_PRESETS.portfolioBalance
) {
  ...
  try {
    expectMoneyClose(endingBalance, expectedEnding, {
      ...options,
      message:
        `Age ${year.age}: Balance continuity violated\n` +
