...
describe('Unified Sanity & Business Logic Audit (The Watchdog)', () => {
  ...
  it('should respect authoritative inputs from Default Profile', () => {
    ...
    const totalPension = ledger.reduce(
      (sum, year) => sum + (year.metrics.detailedCashFlow?.inflows?.pension || 0),
      0
    );
    ...
    const totalSS = ledger.reduce(
      (sum, year) => sum + (year.metrics.detailedCashFlow?.inflows?.socialSecurity || 0),
      0
    );
    ...
    const totalMortgage = ledger.reduce(
      (sum, year) => sum + (year.metrics.detailedCashFlow?.outflows?.mortgage || 0),
      0
    );
